...
import { ArrowRight, ChefHat, Vote, Calendar, AlertTriangle, ShoppingCart } from 'lucide-react';
...
function PlanStatusBanner({ plan, slots, voteStatus }: BannerProps) {
  ...
  if (plan?.finalized_at) {
    return (
      ...
      >
        ...
        <span className="flex-1 text-primary">Meals planned! View grocery list</span>
  ...
  if (votedNames.length > 0) {
    ...
    return (
      <Link
        href="/vote"
        className="flex items-start gap-2 rounded-xl border border-slate-600 bg-slate-800 px-4 py-3 text-sm"
      >
  ...
  return (
    ...
    >
      ...
      <div className="flex flex-col gap-1.5">
        ...
        {!hasAny && (
          <p className="py-1 text-xs italic text-slate-700">Nothing planned</p>
        )}
      </div>
    </div>
  );
